...
$(function () {
  ...
  Wiki.Text.Tag = class {
    ...
    /**
     * @param {String} name - Tag name e.g. diff2, oldid2
     * @param {params} params - Params of the template: {diff2|param1|paramX|….}
     */
  ...
  Wiki.Toolbar = class extends Map {
    ...
    static buttons = {
      ...
      [`as.diffs`]: {
        title   : `Copy selected as {{diff2|…}} wikitext`,
        id      : `as.diffs`,
        label   : `{{Diff2}}`,
        icon    : `code`,
        template: `{{tqb|\n%\n}}`
  ...
  Wiki.HH = class extends Object {
    ...
    /** Convert revisions entries into a Wikitext (diff2s)
     * @since 2.6.0
    ...
    static revisionsToLINKS(revisions){
      ...
      for (let i = 0; i < revisions.length; i++) {
        ...
        wikitext+= `[[Special:Diff/${entry.diff}|[${entry.date}]]]`
      }
      return wikitext
    ...
    static revisionsToTABLE(revisions,  Tag, Table){
      ...
      for (let i = 0; i < revisions.length; i++) {
        ...
        diff    = new Tag(`diff2` , [entry.oldid, entry.date]);
  ...
  if (window.HistoryHelper && window.HistoryHelper.shortcuts) {
    ...
    for (const key in shortcuts) {
      if (Object.hasOwnProperty.call(shortcuts, key)) {
        ...
        if (actionName === `copyAsDiff2s`) {
          shortcuts[key] = `revisions.as.links.to.clipboard`;
          console.warn(`${Wiki.HH.NAME}: copyAsDiff2s action is deprecated after v2.6.0, update your config`)
        }
      }
    }
  }
